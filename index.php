...
else if ($_REQUEST['event'] == 'ONAPPINSTALL')
{
	...
	$result = restCommand('imbot.register', Array(
		...
		'PROPERTIES' => Array(
			'NAME' => 'BotWifigo',
			'WORK_POSITION' => "Bot de atención a clientes",
			'COLOR' => 'RED',
		)
	), $_REQUEST["auth"]);
	$botId = $result['result'];
...
function itrRun($portalId, $dialogId, $userId, $message = '')
{
	if ($userId <= 0)
		return false;
	$biendvenida = "Buenos día gracias por contactar con Wifigo[BR]¿En qué puedo ayudarle?[BR]Puede seleccionar alguno de nuestro temas específicos:";
	$infoInter= "El costo de la instalación es $3,300 la cual incluye antena, modem, cableado y tubo hasta 6 metros[BR]este en caso de que se requiere para que el equipo tenga una mejor recepción[BR]• Al momento de contratar los equipos son de usted si ya no llega a requerir el[BR]servicio con la empresa no pueden retirarle ningún equipo[BR]• La instalación no incluye la mensualidad del paquete que elija[BR]• Contamos con 3 paquetes los cuales son:";
	$menu0 = new ItrMenu(0);
	$menu0->setText($biendvenida. $_REQUEST['event']);
	//$menu0->addItem(1, 'Información sobre internet', ItrItem::sendText('Text message (for #USER_NAME#)'));
	/*
	$menu0->addItem(1, 'Información sobre internet', ItrItem::execFunction(function($context){
		$result = restCommand('imbot.message.add', Array(
			"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
			"MESSAGE" => "/paquetes.png",
			'URL_PREVIEW' => 'Y'
		), $_REQUEST["auth"]);
		//$result = restCommand($_REQUEST["auth"]);
	}, $infoInter));
	*/
	// response time
	$latency = (time()-$_REQUEST['ts']);
	$latency = $latency > 60? (round($latency/60)).'m': $latency."s";
	$menu0->addItem(1, 'Quisiera reportar mi servico', ItrItem::openMenu(1));
	$menu0->addItem(2, 'Cambio de contraseña', ItrItem::execFunction(function($context){
		$result = restCommand('imbot.message.add', Array(
			"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
			"MESSAGE" => "Respueta de repetición",
			"ATTACH" => Array(
				Array("MESSAGE" => "respuesta: ".$_REQUEST['data']['PARAMS']['MESSAGE']),
				Array("MESSAGE" => "tiempo: ".$latency)
			)
		), $_REQUEST["auth"]);
		writeToLog($result, 'Exec function');
	}));
	$menu0->addItem(3, 'Agendar instalación', ItrItem::execFunction(function($context){
		$result = restCommand('imbot.message.add', Array(
			"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
			"MESSAGE" => "Respueta de repetición",
			"ATTACH" => Array(
	...
	//$menu0->addItem(3, 'Otro menu', ItrItem::openMenu(1));
	//$menu0->addItem(0, 'Wait operator answer', ItrItem::sendText('Wait operator answer', true));
	$menu1 = new ItrMenu(1);
	$menu1 -> setText('Reporte de falla: Seleccione una opcion');
	$menu1 -> addItem(1,'Servicio lento', ItrItem::execFunction(function($context){
		/*
		$result = restCommand('imbot.message.add', Array(
			"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
	...
	$menu1 -> addItem(2,'Servicio intermitente', ItrItem::sendText('----'));
	$menu1 -> addItem(3,'Si conexión', ItrItem::sendText('----'));
	$menu1 -> addItem(9,'Volver al menu anterior', ItrItem::openMenu(0));
	
	$menu2 = new ItrMenu(3);
	$menu2->setText('Second menu (#1)');
	$menu2->addItem(2, 'Transfer to queue', ItrItem::transferToQueue('Transfer to queue'));
	$menu2->addItem(3, 'Transfer to user', ItrItem::transferToUser(192, false, 'Transfer to user #1'));
	$menu2->addItem(4, 'Transfer to bot', ItrItem::transferToBot('marta', true, 'Transfer to bot Marta', 'Marta not found :('));
	$menu2->addItem(5, 'Finish session', ItrItem::finishSession('Finish session'));
	$menu2->addItem(6, 'Exec function', ItrItem::execFunction(function($context){
		$result = restCommand('imbot.message.add', Array(
			"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
			"MESSAGE" => 'Function executed (action)',
		), $_REQUEST["auth"]);
	...
	$menu2->addItem(9, 'Back to main menu', ItrItem::openMenu(0));
...
/*
pendiente: Mdificar el funccion getAnswer(), 
sugerencias 1) agregar expresines regulares para validar los
caracteres del nombre ingresando en los case o en su defecto generar una logica con un if comparar el texto de entrada.
2) si la respuesta es difernete de todos los elemtentos de los menus y submeus que ingrese a otra validacion
*/
function getAnswer($command = '', $user) {
	...
	switch (strtolower($command)) {
		...
		default:
			 $arResult = array(
			 'title' => "Estoy atorado",
			 'report'  => "Lo siento, ¿qué es lo que querías? no puedo oírte! -> ".$command . " -> ". $user, 
		  );
		  $arResult = b24BadTasks($user);
			 echo "<script>console.log(".json_encode($_REQUEST).");</script>";
	}
